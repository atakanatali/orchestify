...
export function EditWorkspaceModal({ workspace, onClose }: EditWorkspaceModalProps) {
    const [name, setName] = useState(workspace.name);
    const [defaultBranch, setDefaultBranch] = useState(workspace.defaultBranch);
    const queryClient = useQueryClient();

    const mutation = useMutation({
        mutationFn: () => workspacesApi.update(workspace.id, { name, defaultBranch }),
        onSuccess: () => {
            ...
            >
                ...
                <div className="flex items-center justify-between p-8 border-b border-slate-100 bg-gradient-to-br from-slate-50 to-white">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <h2 className="text-2xl font-black text-slate-900 tracking-tight">Edit Workspace</h2>
                            <p className="text-slate-400 text-sm font-medium">{workspace.repositoryPath.split('/').pop()}</p>
                        </div>
                    </div>
                ...
                <form onSubmit={handleSubmit} className="p-8 space-y-6">
                    ...
                    <div className="pt-4 flex gap-4">
                        <button
                            type="button"
                            onClick={onClose}
                            className="flex-1 px-6 py-4 text-slate-500 rounded-xl font-black uppercase tracking-[2px] text-xs hover:bg-slate-50 transition-all"
                        >
                            Cancel
                        </button>
                        <button
