...
export function EditTaskModal({ boardId, task, onClose }: EditTaskModalProps) {
    ...
    const mutation = useMutation({
        mutationFn: () => tasksApi.update(boardId, task.id, { title, description }),
        onSuccess: () => {
            ...
            >
                ...
                <form onSubmit={handleSubmit} className="p-8 space-y-6">
                    <div className="space-y-2">
                        <label className="text-[10px] font-black text-slate-400 uppercase tracking-[2px]">Task Title</label>
                        <input
                            autoFocus
                            type="text"
                            value={title}
                    ...
                    <div className="space-y-2">
                        ...
                        <textarea
                            ...
                            className="w-full px-5 py-4 bg-slate-50 border border-slate-100 rounded-xl focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 focus:bg-white outline-none transition-all text-slate-700 font-medium resize-none h-32"
